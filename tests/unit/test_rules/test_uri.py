...
class TestUriRule(unittest.TestCase):
    ...
    def test_match(self):
        ...
        rule = sa.rules.uri.URIRule("TEST", pattern=mock_pattern)
    ...
    def test_get_rule_kwargs(self):
        mock_perl2re = patch("sa.rules.uri.sa.regex.perl2re").start()
        data = {"value": "/test/"}
        expected = {"pattern": mock_perl2re("/test/")}
        kwargs = sa.rules.uri.URIRule.get_rule_kwargs(data)
