...
class TestBodyRule(unittest.TestCase):
    ...
    def test_match(self):
        ...
        rule = sa.rules.body.BodyRule("TEST", pattern=mock_pattern)
    ...
    def test_get_rule_kwargs(self):
        ...
        kwargs = sa.rules.body.BodyRule.get_rule_kwargs(data)
...
class TestRawBodyRule(unittest.TestCase):
    ...
    def test_match(self):
        mock_pattern = Mock(**{"match.return_value": True})
        rule = sa.rules.body.RawBodyRule("TEST", pattern=mock_pattern)
        result = rule.match(self.mock_msg)
